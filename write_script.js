...
// 2. Add Marquee Keyframes to the style block
const keyframesCSS = `
  @keyframes myMarquee {
...
`;
html = html.replace('</style>', keyframesCSS + '\n</style>');

// Fix the marquee div
const originalMarqueeMatch = html.match(/<div class="whitespace-nowrap flex[^>]*>/);
if (originalMarqueeMatch) {
  html = html.replace(originalMarqueeMatch[0], '<div class="dynamic-marquee">');
}

// 3. Fix Accordion Content
...
const oldContentRegex1 = /<div class="pb-6 text-sm text-gray-800 leading-relaxed space-y-2 accordion-content">[\s\S]*?(?=<\/div><\/div><div class="border-b border-border)/;
const oldContentRegex2 = /<div class="pb-6 text-sm text-gray-800 leading-relaxed space-y-2 accordion-content">[\s\S]*?(?=<\/div><\/div><div class="border-b border-border)/g;

// To precisely replace, we split by the accordion headers:
const sections = html.split('class="w-full flex items-center justify-between py-5 text-left focus:outline-none"');

// Note: section[0] ends before the first button. 
// section[1] is RENDIMIENTO
// section[2] is BATERIA
// section[3] is MODOS DE INFLADO
// section[4] is ESPECIFICACIONES
// section[5] is ENVÍO Y DEVOLUCIONES

if (sections.length === 6) {
  function replaceAccordionContent(sectionCode, newContent) {
    return sectionCode.replace(/<div class="pb-6 text-sm text-gray-800 leading-relaxed space-y-[234] accordion-content">[\s\S]*?<\/div>$/, newContent + '</div>');
  }

  // A more robust regex replacement, we will target the actual inner DOM string. Let's just use string replacement of the specific wrappers.
}

fs.writeFileSync('fix_script.js', \`
const fs = require('fs');
let html = fs.readFileSync('NovaHogarCR_LandingPage_Estatica.html', 'utf8');
...
// <div class="pb-6 text-sm text-gray-800 leading-relaxed space-y-2 accordion-content">
//    <div class="flex justify-between border-b border-gray-100 pb-1"><span class="font-bold">Llanta de carro...
// Let's explicitly replace these blocks by matching strings with wildcards or just substring replacements

html = html.replace(/<div class="pb-6 text-sm text-gray-800 leading-relaxed space-y-2 accordion-content">[\\s\\S]*?<\\/div>\\s*<\\/div>\\s*<div class="border-b border-border last:border-b-0">/, content1 + '</div><div class="border-b border-border last:border-b-0">');

// We have 5 accordion buttons
...
if(parts.length >= 6) {
    ...
    // 5: ENVÍO Y DEVOLUCIONES (this one might not end with the next accordion, but with a closing div)
